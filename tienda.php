...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $productos_result = $mysql->query("
        ...
        FROM productos 
        LEFT JOIN imagenes ON productos.imagen_id = imagenes.codigo
        JOIN productos_categorias ON productos.codigo = productos_categorias.producto_id
        WHERE productos.en_venta = 1 AND productos_categorias.categoria_id = $categoria_id
    ");
}
?>
    ...
    <main>
        <div class="sidebar">
            ...
            <ul class="category-list-shop">
                <?php while ($categoria = $result->fetch_assoc()) { ?>
                    <li><a href="tienda.php?categoria_id=<?php echo htmlspecialchars($categoria['codigo']); ?>"><?php echo htmlspecialchars($categoria['nombre']); ?></a></li>
                <?php } ?>
            </ul>
        </div>
        ...
        <div class="main-products">
            <div class="product-items">
                <?php if ($categoria_id > 0 && $productos_result->num_rows > 0) { 
